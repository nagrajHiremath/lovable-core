...
const getPreviewUrlKey = (projectId: string) => `${PREVIEW_URL_KEY}_${projectId}`;

export function PreviewPanel({ projectId, runtimeError, onDismiss, onFix }: PreviewPanelProps) {
  const [previewUrl, setPreviewUrl] = useState<string | null>(() => {
    // Load from localStorage on mount
    return localStorage.getItem(getPreviewUrlKey(projectId));
  });
  const [isDeploying, setIsDeploying] = useState(false);
  const { toast } = useToast();

  // Reload the stored preview URL whenever the project changes
  useEffect(() => {
    setPreviewUrl(localStorage.getItem(getPreviewUrlKey(projectId)));
  }, [projectId]);
