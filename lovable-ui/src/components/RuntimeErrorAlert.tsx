...
export function RuntimeErrorAlert({ error, onDismiss, onFix }: RuntimeErrorAlertProps) {
    ...
    return (
        <div className="absolute bottom-4 right-4 z-50 animate-in slide-in-from-bottom-5 fade-in duration-300">
            <div className="w-[400px] bg-popover border border-red-500/20 rounded-xl shadow-2xl overflow-hidden">
                {/* Header */}
                ...
                {/* Footer */}
                <div className="p-3 bg-muted/60 flex items-center justify-between border-t border-border">
                    ...
                    <Button
                        size="sm"
                        onClick={() => onFix(error)}
                        className="bg-primary hover:bg-primary/90 text-primary-foreground border-none h-8 px-4 text-xs font-medium gap-2 shadow-lg shadow-primary/20"
                    >
                        <Wrench className="w-3.5 h-3.5" />
                        Fix issues
                        <kbd className="hidden sm:inline-flex h-4 items-center gap-1 rounded border border-primary-foreground/30 bg-primary-foreground/20 px-1 font-mono text-[9px] font-medium text-primary-foreground opacity-100">
                            F
                        </kbd>
                    </Button>
                </div>
