...
import { api, buildFileTree, FileNode, OPEN_TABS_KEY, ACTIVE_TAB_KEY } from "@/lib/api";
...
// Helper to collect every file path currently in the tree
function flattenFilePaths(files: FileNode[]): string[] {
  const paths: string[] = [];
...
export function CodePanel({ projectId, updatedFiles }: CodePanelProps) {
  ...
  // Add newly-created files from streaming into the file tree as they arrive
  useEffect(() => {
    if (updatedFiles.size === 0) return;

    setFiles((prev) => {
      const existingPaths = flattenFilePaths(prev);
      const newPaths = Array.from(updatedFiles.keys()).filter((path) => !existingPaths.includes(path));
      if (newPaths.length === 0) return prev;

      return buildFileTree([...existingPaths, ...newPaths].map((path) => ({ path })));
    });
  }, [updatedFiles]);
