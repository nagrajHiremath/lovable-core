...
export function ProjectView() {
  ...
  // Send the prompt typed on the landing page (before the user was authenticated), once.
  useEffect(() => {
    if (isLoadingHistory || initialMessageSentRef.current) return;
  ...
  }, [isLoadingHistory, location, messages.length, handleSendMessage, navigate]);
...
Please analyze this error and fix the code to resolve it.`;
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden bg-background">
              ...
              {project.role && (
                <span className={cn(
                  ...
                  project.role === 'OWNER' ? "bg-primary/10 text-primary" :
                    project.role === 'DEVELOPER' ? "bg-amber-500/10 text-amber-600" :
      ...
      <UserDashboardModal open={isUpgradeModalOpen} onOpenChange={setIsUpgradeModalOpen} />
    </div >
  );
}
